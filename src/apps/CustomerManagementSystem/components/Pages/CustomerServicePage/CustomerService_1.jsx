...
const CustomerService_1 = () => {
  ...
  const fetchUserStatus = async (userId) => {
    try {
      const response = await fetch(
        `https://grozziieget.zjweiting.com:3091/CustomerService-Chat/api/dev/user/status/${userId}`
      );
      if (response.ok) {
  ...
  const fetchUserByChatId = async () => {
    ...
    try {
      const response = await axios.get(
        `https://grozziieget.zjweiting.com:3091/CustomerService-Chat/api/dev/messages/${selectedCustomerChat?.chatId}`
      );
      ...
      <div className="  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 md:gap-8 mx-3 md:mx-12 my-12 text-gray-600 bg-white">
        ...
        <div className=" h-[80vh] shadow-lg rounded-lg py-5 md:px-10">
          <div className="flex items-center justify-between">
            <div className="flex justify-start">
              <img
                className="h-8 w-8 ml-3 rounded-full  shadow-slate-900"
                src={
                  user?.image
                    ? user?.image
            ...
            <div className=" bg-red-900 rounded-full">
              {connected ? (
                <SiSocketdotio className="bg-green-400 text-lg rounded-full"></SiSocketdotio>
              ) : (
          ...
          <div className=" overflow-y-scroll h-[60vh]">
            {currentCustomer.map((element, index) => {
              return (
                <div
                  key={index}
                  className={`text-sm  ml-2 px-3 cursor-pointer rounded-l-lg ${
                    currentUser?.userId === element?.userId
                      ? "bg-green-300"
                      : ""
                  }`}
                >
                  <div
                    onClick={() => handleToSelectCustomer(element)}
                    className="flex justify-between items-center mx-1 my-1 cursor-pointer"
                  >
                    ...
                    <div>
                      ...
                      {newMessagesList?.filter(
                        (sms) => sms?.sentBy === element?.userId
                      )?.length > 0 && (
                        <div className="bg-yellow-400 px-2 rounded-full border-2 text-black font-semibold">
                          {" "}
                          {
                            newMessagesList?.filter(
                              (sms, index, self) =>
        ...
        <div className=" h-[80vh] shadow-lg rounded-lg relative  mt-10 md:mt-0 md:px-10">
          <div className="flex justify-around ">
            {activeStatus === "ONLINE" ? (
              <p className="font-semibold text-green-600 px-2 py-0  border-4 rounded-full border-green-600">
                {currentUser?.userId}
              </p>
            ) : (
              <p className="font-semibold text-red-600 px-2 py-0  border-4 rounded-full border-red-600">
                {currentUser?.userId}
              </p>
            )}

            <p className="bg-[#004368] text-gray-200 px-2 rounded-b-lg py-1">
              Text from app
            </p>
            <p className="font-semibold">
              {currentUser?.customerServiceName
                ? currentUser?.customerServiceName
                : `No Name(${currentUser?.userId})`}
            </p>
          </div>
          ...
          <div
            className=" overflow-y-scroll h-[60vh] mb-10 text-start"
