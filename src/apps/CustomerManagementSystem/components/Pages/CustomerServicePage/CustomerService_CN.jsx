...
const CustomerService_CN = () => {
  ...
  const fetchUserStatus = async (userId) => {
    try {
      const response = await fetch(
        `https://jiapuv.com:3091/CustomerService-ChatCN/api/dev/user/status/${userId}`
      );
      if (response.ok) {
  ...
  const fetchUserByChatId = async () => {
    ...
    try {
      const response = await axios.get(
        `https://jiapuv.com:3091/CustomerService-ChatCN/api/dev/messages/${selectedCustomerChat?.chatId}`
      );
