...
const EventProducts = () => {
  ...
  const { allEventProduct, setAllEventProduct, setProduct } =
  ...
  const handleToDelete = async (productId) => {
    ...
    try {
      ...
      await axios.delete(
        `https://grozziieget.zjweiting.com:8033/tht/eventProducts/delete/${productId}`
      );
      toast.success(`One MallProduct deleted successfully`);
      const restProduct = eventProduct.filter(
  ...
  const filteredAllProduct = eventProduct.filter(
    (request) =>
      request?.productName
        .toLowerCase()
  ...
  return (
    <div className="text-gray-800">
      <h1 className="my-5 text-lg font-semibold">Event product list</h1>
      <div className="p-4">
        ...
        <div className=" h-screen overflow-y-scroll">
          <div className="mx-2 my-3 grid grid-cols-7  text-start text-lg font-semibold bg-slate-300 px-2 py-2  sticky top-0">
            <div className=" col-span-6 grid grid-cols-8">
              <p>Image</p>
              <p>Product Name</p>
              <p className="">Model No</p>
              <p className="">Country</p>
              <p className="">Id</p>
              <p className="">Show Type</p>
              <p className="">Show Mark</p>
          ...
          {loading ? (
            ...
            filteredAllProduct?.map((product, index) => (
              ...
              <div className="  mx-2 my-3 grid grid-cols-7  text-start bg-slate-200 hover:bg-yellow-100 cursor-pointer rounded-lg px-2 py-2">
                <Link
                  key={index}
                  to={`/customer-management-system/admin/eventProduct/details/${product?.modelNumber}}`}
                  onClick={() => setProduct(product)}
                  className=" col-span-6 grid grid-cols-8 text-gray-700 hover:text-gray-800"
                >
                  <img
                    className=" h-10 w-10 rounded-full"
                    src={`https://grozziieget.zjweiting.com:8033/tht/eventProductImages/${product.productImg}`}
                    alt={product.productName}
                  ></img>

                  <p>{product?.productName}</p>
                  <p className="">{product?.modelNumber}</p>
                  <p className="">{product?.productCountryName}</p>
                  <p className="">{product?.id}</p>
                  <p className="">{product?.productImgRemark}</p>
                  <p className="">{product?.mark}</p>
                  <p className="">{product?.slideImageMark}</p>
                </Link>
      ...
      <Link to="/customer-management-system/admin/eventProduct/add">
        <button
