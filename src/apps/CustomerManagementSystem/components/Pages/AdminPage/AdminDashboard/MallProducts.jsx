...
const MallProducts = () => {
  const { allMallProduct, setAllMallProduct, setProduct } =
  ...
  const handleToDelete = async (productId) => {
    ...
    try {
      ...
      await axios.delete(
        `https://grozziieget.zjweiting.com:8033/tht/mallProducts/delete/${productId}`
      );
  ...
  return (
    <div className="text-gray-800">
      <h1 className="my-5 text-lg font-semibold">Mall product list</h1>
      <div className="p-4">
        ...
        <div className=" h-screen overflow-y-scroll">
          ...
          {loading === true ? (
            ...
            filteredAllProduct?.map((product, index) => (
              <div
                key={index}
                className="mx-2 my-3 grid grid-cols-7 text-start bg-slate-200 hover:bg-yellow-100 cursor-pointer rounded-lg px-2 py-2"
              >
                <Link
                  to={`/customer-management-system/admin/mallProduct/details/${product?.modelNumber}}`}
                  onClick={() => setProduct(product)}
                  className=" col-span-6 grid grid-cols-6 text-gray-700 hover:text-gray-800 font-normal"
                >
                  <img
                    className=" h-10 w-10 rounded-full"
                    src={`https://grozziieget.zjweiting.com:8033/tht/mallProductImages/${product.productImg}`}
      ...
      <Link to="/customer-management-system/admin/mallProduct/add">
        {}
        <button
