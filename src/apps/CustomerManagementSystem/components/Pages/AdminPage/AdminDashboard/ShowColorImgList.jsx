...
const ShowColorImgList = ({ modelNumber, productId, categoryImage }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://grozziieget.zjweiting.com:8033/tht/colorImg/productColor/${productId}/${categoryImage}`,
      )
  ...
  const handleToDeleteColorInfo = (id) => {
    ...
    const confirmed = window.confirm(
      "Are you sure you want to delete this color image information?",
    );
    if (!confirmed) {
    ...
    axios
      .delete(
        `https://grozziieget.zjweiting.com:8033/tht/colorInfo/delete/${id}`,
      )
      ...
      .catch((error) => {
        console.error(
          "An error occurred while deleting color information:",
          error,
        );
        toast.error(
          "An error occurred while deleting color information:",
  ...
  const handleModalSave = async (colorId) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://grozziieget.zjweiting.com:8033/tht/colorImg/edit/${colorId}`,
        updatedColorInfo,
      );
      ...
      console.error(
        "An error occurred while updating color information:",
        error,
      );
    ...
    <div>
      ...
      {loading ? (
        ...
        <span className="text-xl font-bold text-red-400">
          No Color Images Available for {modelNumber}
        </span>
      ) : (
        colorImages?.map((colorImage, index) => (
          <div
            key={index}
            className="mx-2 my-3 flex justify-between items-center text-start bg-slate-200 hover:bg-yellow-100  rounded-lg px-2 py-2"
          >
            <img
              className=" h-10 w-10 rounded-full"
              src={`https://grozziieget.zjweiting.com:8033/tht/colorImages/${colorImage.colorImage}`}
              alt={colorImage.colorName}
            ></img>
            ...
            <p>{modelNumber}</p>
            <p>{colorImage?.colorName}</p>
            <p>{colorImage?.colorProductQuantity}</p>
            <p className="">{colorImage?.typeName}</p>
            <p className="">{colorImage?.colorProductPrice}</p>
            <FiEdit
              onClick={() => handleEditColorInfo(colorImage)}
              className="hover:cursor-pointer hover:text-2xl text-center"
            ></FiEdit>
            <RiDeleteBin7Line
              onClick={() => handleToDeleteColorInfo(colorImage?.id)}
              className="hover:cursor-pointer hover:text-2xl"
            ></RiDeleteBin7Line>
            {isModalVisible && (
              <div className="fixed  z-50 inset-0  mx-auto rounded-lg h-screen w-screen my-auto  bg-gray-900 bg-opacity-50">
                <div className="bg-white w-6/12 h-5/6 mt-12 mx-auto p-2 px-8 text-center rounded-lg">
                  <h2 className="text-lg font-bold mb-2">
                    Edit Product color image information
                  </h2>
