...
const ShowBackgroundImg = () => {
  ...
  useEffect(() => {
    const apiUrl = `${baseUrl}/tht/backgroundImgs/${categoryName}`;
    axios
      .get(apiUrl)
      .then((response) => {
  ...
  const handleToDelete = async (id) => {
    try {
      await axios.delete(`${baseUrl}/tht/backgroundImgs/delete/${id}`);
  ...
  const handleToDownload = (icon) => {
    const imageURL = `${baseUrl}/tht/backgroundImgs/${icon}`; // Replace with your image URL
    fetch(imageURL)
  ...
  return (
    <div className=" min-h-screen">
      <h1 className="text-3xl font-bold text-yellow-400 my-10">
        Available icons for{" "}
        <span className="text-teal-400">{categoryName}</span> category
      </h1>
      {loading ? (
          ...
          {allIcons?.map((element, index) => {
            return (
              <div className=" relative border-2">
                <AiOutlineDownload
                  onClick={() => handleToDownload(element?.image)}
                  className=" absolute top-0 hover:cursor-pointer text-green-500"
                ></AiOutlineDownload>
                <MdDelete
                  onClick={() => handleToDelete(element?.id)}
                  className=" absolute right-0 hover:cursor-pointer text-red-500"
                ></MdDelete>
                <img
                  key={index}
                  id="myDiv"
                  className=" inline-block w-28 h-28"
                  src={`${baseUrl}/tht/backgroundImgs/${element.image}`}
                  alt="Icon"
                ></img>
              </div>
            );
