...
const UserList = ({
  ...
  const handleToSelectedCustomer = (element) => {
    ...
    setMessages(
      customerMessages.filter((message) => message?.id === selectedCustomer?.id)
    );
  };
      ...
      <div className=" h-[80vh] shadow-lg rounded-lg py-5 md:px-10">
        <div className="flex items-center justify-start">
          <img
            ...
            src={currentUser ? currentUser?.image : user?.photoURL}
            alt=""
          />
        ...
        <div className=" overflow-y-scroll h-[60vh]">
          {customerName.map((element, index) => {
            return (
              <div
                key={index}
                onClick={() => handleToSelectedCustomer(element)}
                className="text-sm  ml-2 px-3"
              >
