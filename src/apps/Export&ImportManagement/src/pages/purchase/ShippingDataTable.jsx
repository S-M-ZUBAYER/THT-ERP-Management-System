...
const ShippingDataTable = ({
  ...
  const handleChargeChange = (index, field, value) => {
    ...
    updatedCharges[index][field] =
      field === "amountUSD" ? parseFloat(value) || 0 : value;
    if (field === "amountUSD") {
  ...
  useEffect(() => {
    setFormData({
      ...formData,
      destination: `${transportPort}, ${transportCountry}`,
    });
  }, [transportCountry, transportPort]);
  useEffect(() => {
    const totalUSD = formData.charges
      .reduce((acc, charge) => acc + parseFloat(charge.amountUSD || 0), 0)
      .toFixed(2);
  ...
  useEffect(() => {
    const totalTK = formData.charges
      .reduce((acc, charge) => acc + parseFloat(charge.amountBDT || 0), 0)
      .toFixed(2);
  ...
  return (
    <div className="p-4">
      <h1 className="text-3xl underline font-bold mb-6 text-center text-gray-800">
        Sea Freight Shipping Details Form
      </h1>
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        ...
        <div>
          ...
          <input
            type="text"
            name="destination"
            value={`${transportPort ? transportPort : ""}, ${
            ...
            <tr key={index}>
              ...
              <td className="border border-gray-300 p-2">
                <input
                  ...
                  onChange={(e) =>
                    handleChargeChange(index, "amountUSD", e.target.value)
                  }
                  className="w-full p-2 border border-gray-300 rounded"
                />
              </td>
              ...
              <td className="border border-gray-300 p-2">
                <button
                  ...
                  onClick={() => {
                    const updatedCharges = formData.charges.filter(
                      (_, i) => i !== index
                    );
                    setFormData({ ...formData, charges: updatedCharges });
                  }}
